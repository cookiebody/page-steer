...
- **MultiPageSteer safety** - Disabled `ScriptExecutionTool` for `MultiPageSteer`.
...
- **Robust abort handling** - Rewrote the aborting system; sync tools, loop execution, and LLM clients now correctly respect task abort signals (`ctx.signal`). Also decoupled `AbortError` from `InvokeError` in `@page-steer/llms`.
- **Claude Opus 4.8 support** - Added support for Claude Opus 4.8 model.

### Improvements

- **Concurrency guard** - Prevented concurrent `execute()` calls on a single PageSteer/Core instance to avoid race conditions.
- **Model recommendations refresh** - Updated default and tested model list recommendations.
- **Test coverage** - Added comprehensive Vitest unit tests for the `@page-steer/llms` package.
...
- **Beta MCP support** - New `@page-steer/mcp` package lets MCP clients such as Claude Desktop and Copilot control the browser through the Page Steer extension
...
- **`data-browser-use-ignore` → `data-page-steer-ignore`** - DOM ignore attribute renamed to match the project identity
- **Config types restructured** - `PageSteerConfig` split into `AgentConfig` + `PageSteerCoreConfig`; config definitions moved from `config/index.ts` to `types.ts`
...
- Exported `InvokeError` and `InvokeErrorType` as values (not just types) from `@page-steer/llms`
- Exported `SupportedLanguage` type from `@page-steer/core`
...
- **Lifecycle: `stop()` vs `dispose()`** - New `stop()` method to cancel the current task while keeping the agent reusable. `dispose()` is now terminal — a disposed agent cannot be reused. This affects both `PageSteerCore` and `PanelAgentAdapter`.
...
PageSteer is now ready for production use. The API is stable and breaking changes will follow semantic versioning.

### Features

#### Core

- **PageSteer** - Main entry class with built-in UI Panel
- **PageSteerCore** - Headless agent class for custom UI or programmatic use
...
| Package                       | Description                        |
| ----------------------------- | ---------------------------------- |
| `page-steer`                  | Main entry with UI Panel           |
| `@page-steer/core`            | Core agent logic without UI        |
| `@page-steer/llms`            | LLM client with retry logic        |
| `@page-steer/page-controller` | DOM operations and visual feedback |
| `@page-steer/ui`              | Panel and i18n                     |
...
- See [Limitations](https://cookiebody.github.io/docs/page-steer/docs/introduction/limitations) for details
