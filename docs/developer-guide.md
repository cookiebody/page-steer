...
- **Page Steer** (`packages/page-steer/`) - Main entry with built-in UI Panel (npm: `page-steer`)
- **MCP** (`packages/mcp/`) - MCP server for browser control via Page Steer extension (npm: `@page-steer/mcp`)
- **Core** (`packages/core/`) - Core agent logic without UI (npm: `@page-steer/core`)
...
- **Ollama example** (tested on 0.15 + qwen3:14b, RTX3090 24GB):
    ...
    > @see https://cookiebody.github.io/docs/page-steer/docs/features/models#ollama for configuration
...
- Start and serve a local `iife` script
    ...
    ```bash
    npm run dev:demo # Serving IIFE with auto rebuild at http://localhost:5174/page-steer.demo.js
    ```

- Add a new bookmark

    ```javascript
    javascript:(function(){var s=document.createElement('script');s.src=`http://localhost:5174/page-steer.demo.js?t=${Math.random()}`;s.onload=()=>console.log(%27PageSteer ready!%27);document.head.appendChild(s);})();
    ```

- Click the bookmark on any page to load Page Steer
