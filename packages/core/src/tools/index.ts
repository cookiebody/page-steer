...
/**
 * Internal tool definition that has access to PageSteer `this` context
 */
export interface PageSteerTool<TParams = any> {
	// name: string
	description: string
	inputSchema: z.ZodType<TParams>
	execute: (this: PageSteerCore, args: TParams, ctx: ToolContext) => Promise<string>
}

export function tool<TParams>(options: PageSteerTool<TParams>): PageSteerTool<TParams> {
	return options
}
 ...
 */
export const tools = new Map<string, PageSteerTool>()

tools.set(
	'done',
	tool({
		description:
			'Complete task. Text is your final response to the user — use markdown formatting (headings, lists, code blocks) for structured content to improve readability. Keep it concise unless the user explicitly asks for detail.',
		inputSchema: z.object({
			text: z.string(),
			success: z.boolean().default(true),
		}),
		execute: async function (this: PageSteerCore, input) {
...
tools.set(
	...
	tool({
		...
		execute: async function (this: PageSteerCore, input, { signal }) {
