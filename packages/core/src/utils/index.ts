...
if (!_global.__PAGE_STEER_IDS__) {
	_global.__PAGE_STEER_IDS__ = []
}

const ids = _global.__PAGE_STEER_IDS__
...
export async function fetchLlmsTxt(url: string): Promise<string | null> {
	...
	try {
		console.debug(chalk.gray(`[llms.txt] Fetching ${endpoint}`))
		const res = await fetch(endpoint, { signal: AbortSignal.timeout(3000) })
		if (res.ok) {
			result = await res.text()
			console.debug(chalk.green(`[llms.txt] Found (${result.length} chars)`))
			if (result.length > 1000) {
				console.debug(chalk.yellow(`[llms.txt] Truncating to 1000 chars`))
