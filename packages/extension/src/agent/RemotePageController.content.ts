...
export function initPageController() {
	...
	intervalID = window.setInterval(async () => {
		try {
			...
			// Clean up interval to stop polling.
			if (error instanceof Error && error.message.includes('Extension context invalidated')) {
				console.debug(
					'[RemotePageController.ContentScript]: Extension context invalidated, cleaning up.'
				)
				if (intervalID !== null) {
		...
		switch (action) {
			...
			case 'execute_javascript':
				pc[methodName](...(payload || []))
					...
					.catch((error: any) => {
						// If extension context is gone, there's no one to respond to
						if (error instanceof Error && error.message.includes('Extension context invalidated')) {
							return
						}
						sendResponse({
