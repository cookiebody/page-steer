import type { BrowserState } from '@page-steer/page-controller'
...
export class RemotePageController {
	...
	async getBrowserState(): Promise<BrowserState> {
		...
		if (!this.currentTabId || !isContentScriptAllowed(currentUrl)) {
			...
			// Content script may fail (e.g. extension context invalidated, page not ready)
			if (!result || typeof result !== 'object' || result.content === undefined) {
				browserState = {
					...
					header: '',
					content:
						'(Page content unavailable. The page may need to be refreshed, or the content script failed to load. Try refreshing the page or navigating to it again.)',
					footer: '',
				}
			} else {
				browserState = result
