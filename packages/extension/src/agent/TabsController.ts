...
export class TabsController {
	 ...
	 * The background service worker may become inactive (MV3), causing disconnects.
	 * We use exponential backoff to avoid burning retries during suspension.
	 */
	private connectTabEvents() {
		try {
			...
			if (error instanceof Error && error.message.includes('Extension context invalidated')) {
				debug('Extension context invalidated, stopping port reconnection')
				return
			}
			throw error
		}
		...
		this.port.onDisconnect.addListener(() => {
			...
			const delay = Math.min(500 * 2 ** this.portRetries, 32_000)
			debug(`port disconnected, reconnecting in ${delay}ms (retry ${this.portRetries + 1}/7)`)
			this.portRetries++
			setTimeout(() => {
				if (!this.disposed) this.connectTabEvents()
			}, delay)
		})
	}
