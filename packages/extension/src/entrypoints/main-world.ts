...
export default defineUnlistedScript(() => {
	...
	const execute: Execute = async (task, config) => {
		...
		const promise = new Promise<ExecutionResult>((resolve, reject) => {
			function handleMessage(e: MessageEvent) {
				...
				if (data.channel !== 'PAGE_STEER_EXT_RESPONSE') return
		...
		window.postMessage(
			{
				channel: 'PAGE_STEER_EXT_REQUEST',
				id,
				action: 'execute',
				payload: {
					task,
	...
	;(window as any).PAGE_STEER_EXT_VERSION = __VERSION__
	;(window as any).PAGE_STEER_EXT = {
		version: __VERSION__,
		execute,
		stop,
	}
