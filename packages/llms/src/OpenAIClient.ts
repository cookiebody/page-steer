...
export class OpenAIClient implements LLMClient {
	...
	async invoke(
		...
		const bodyStr = JSON.stringify(finalRequestBody)
		...
		try {
			...
			console.error(
				`[LLM] ${isTimeout ? 'Timeout' : 'Network error'} (request body: ${sizeKB}KB):`,
				error
			)
			throw new InvokeError(
				InvokeErrorTypes.NETWORK_ERROR,
				`Network request failed (body: ${sizeKB}KB${isTimeout ? ', timeout' : ''})`,
				error
			)
		}
		clearTimeout(timeoutId)
