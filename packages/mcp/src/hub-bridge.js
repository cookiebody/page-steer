...
const STORE_URL = `https://chromewebstore.google.com/detail/page-steer-ext/${EXT_ID}`
...
export class HubBridge {
		...
		return new Promise((resolve, reject) => {
			this.#httpServer.on('error', (/** @type {NodeJS.ErrnoException} */ err) => {
				if (err.code === 'EADDRINUSE') {
					reject(
						new Error(`Port ${this.port} is in use. Another Page Steer MCP server may be running.`)
					)
				} else {
					reject(err)
				}
			...
			this.#httpServer.listen(this.port, LOOPBACK_HOST, () => {
				console.error(`[page-steer-mcp] HTTP + WS on http://${LOOPBACK_HOST}:${this.port}`)
				resolve()
			})
		})
	}
	...
	/**
	 * Send a fine-grained command to the extension.
	 * Used for direct browser control (navigate, click, get_content, etc.)
	...
	async sendCommand(command, params = {}) {
		if (!this.connected) throw new Error('Hub is not connected. Is the extension running?')
		if (this.#pendingTask) throw new Error('Agent is busy with another operation.')

		return new Promise((resolve, reject) => {
			this.#pendingTask = { resolve, reject }
			this.#hub.send(JSON.stringify({ type: 'command', command, params }))
		})
	}
	...
	#onConnection(ws) {
		...
		this.#hub = ws
		console.error('[page-steer-mcp] Hub connected')
		...
		ws.on('close', () => {
			console.error('[page-steer-mcp] Hub disconnected')
