...
export class PageController extends EventTarget {
	...
	async updateTree(): Promise<string> {
		...
		const blacklist = [
			...
			...Array.from(document.querySelectorAll('[data-page-steer-not-interactive]')),
		]
		...
		this.simplifiedHTML =
			this.config.outputFormat === 'a11y'
				? dom.flatTreeToA11yString(this.flatTree)
				: dom.flatTreeToString(
						this.flatTree,
	...
	async clickElement(index: number): Promise<ActionResult> {
		try {
			...
			const blocker = await checkElementBlocker(element)
			if (blocker) {
				return {
					success: false,
					message: `❌ Element [${index}] (${elemText ?? index}) is covered by ${blocker}`,
				}
			}
