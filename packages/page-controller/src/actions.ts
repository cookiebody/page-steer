...
/**
 * Describe an element concisely for LLM error messages.
 * Format: <tag.class#id> or <tag.class> or <tag>
 */
...
/**
 * Check whether the target element's click point is covered by an unrelated element.
 * Uses document.elementFromPoint to detect overlapping elements (modals, overlays, tooltips, etc.)
 *
 * @returns A descriptive string of the blocker if covered, or `null` if not blocked.
 */
export async function checkElementBlocker(element: HTMLElement): Promise<string | null> {
	const rect = element.getBoundingClientRect()

	// Edge case: element has zero size
	if (rect.width === 0 && rect.height === 0) {
		return 'Element has zero size and cannot be clicked.'
	}

	const x = rect.left + rect.width / 2
	const y = rect.top + rect.height / 2

	// Edge case: element center is outside viewport
	const doc = element.ownerDocument
	const win = doc.defaultView || window
	if (x < 0 || y < 0 || x > win.innerWidth || y > win.innerHeight) {
		return 'Element is outside the visible viewport.'
	}

	// Temporarily disable mask overlay so elementFromPoint hits real DOM
	await enablePassThrough()
	const hitElement = doc.elementFromPoint(x, y)
	...
	// Not blocked if hit element IS the target
	if (hitElement === element) {
		return null
	}

	// Not blocked if hit element is a descendant of the target
	if (element.contains(hitElement)) {
		return null
	}

	// Not blocked if target is a descendant of the hit element (click will bubble up)
	if (hitElement.contains(element)) {
		return null
	}

	// Check shadow DOM: if the hit element's shadow root contains the target
	if (hitElement.shadowRoot && hitElement.shadowRoot.contains(element)) {
		return null
	}

	// Check label association: <label for="id"> linked to the target
	if (hitElement.tagName === 'LABEL') {
		const label = hitElement as HTMLLabelElement
		// Explicit for= association
		if (label.htmlFor && element.id && label.htmlFor === element.id) {
			return null
		}
	...
	// The reverse: target is a label for the hit element
	if (element.tagName === 'LABEL') {
		const label = element as HTMLLabelElement
		if (label.htmlFor && hitElement.id && label.htmlFor === hitElement.id) {
			return null
		}
		if (label.contains(hitElement)) {
			return null
		}
	}

	// Blocked by an unrelated element
	return `${describeElement(hitElement)} at its click point. Dismiss or interact with the covering element first.`
}
