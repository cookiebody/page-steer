...
export class Panel {
	...
	#createWrapper(): HTMLElement {
		...
		wrapper.id = 'page-steer-runtime_agent-panel'
		wrapper.className = styles.wrapper
		wrapper.setAttribute('data-browser-use-ignore', 'true')
		wrapper.setAttribute('data-page-steer-ignore', 'true')
	...
	/** Bind copy button click handlers after rendering */
	#bindCopyButtons(): void {
		const buttons = this.#historySection.querySelectorAll(`.${styles.copyButton}`)
		for (const btn of buttons) {
			if ((btn as any).__copyBound) continue
			;(btn as any).__copyBound = true
			btn.addEventListener('click', (e) => {
				e.stopPropagation()
				const content = (btn as HTMLElement).getAttribute('data-copy-content') || ''
