...
- **wouter** with browser routing (`base: "/docs/page-steer"`)
...
Uses wouter browser routing with base path for GitHub Pages deployment at `https://cookiebody.github.io/docs/page-steer/`.
...
- Header and Footer live in `router.tsx` **outside** `<Switch>`, so they always see the root router context (`base="/docs/page-steer"`)
- Docs pages are nested via `<Route path="/docs" nest>`, which creates a child context (`base="/docs/page-steer/docs"`)
