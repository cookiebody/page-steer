...
export default function CustomUIDocs() {
	...
	return (
		<div>
			...
			<p className="text-xl text-gray-600 dark:text-gray-300 mb-8 leading-relaxed">
				{isZh
					? 'PageSteer 的核心逻辑（PageSteerCore）和 UI 完全解耦，通过事件通讯。你可以用自己的 UI 替换内置 Panel。'
					: 'PageSteer core logic (PageSteerCore) is fully decoupled from UI through events. You can replace the built-in Panel with your own UI.'}
			</p>
			...
			<section className="mb-10">
				...
				<p className="text-gray-600 dark:text-gray-400 mb-4">
					{isZh
						? 'PageSteer 由三个独立模块组成，可自由组合：'
						: 'PageSteer consists of three independent modules that can be freely combined:'}
				</p>
				<ul className="list-disc list-inside text-gray-600 dark:text-gray-400 space-y-2 mb-4">
					<li>
						<strong>PageSteerCore</strong> -{' '}
			...
			<section className="mb-10">
				...
				<p className="text-gray-600 dark:text-gray-400 mb-4">
					{isZh
						? 'PageSteerCore 提供两种不同性质的事件流，方便 UI 渲染：'
						: 'PageSteerCore provides two distinct event streams for UI rendering:'}
				</p>
  ...
  | { type: 'error'; message: string }`}
			...
			<section className="mb-10">
				...
				<CodeEditor
					language="tsx"
					code={`function useAgent(agent: PageSteerCore) {
  ...
  return { status, history, activity }
			...
			<section className="mb-10">
				...
				<p className="text-gray-600 dark:text-gray-400 mb-4">
					{isZh
						? '参考内置 PageSteer 的实现方式，用自定义 UI 替换 Panel：'
						: 'Following the built-in PageSteer pattern, replace Panel with custom UI:'}
				</p>
				<CodeEditor
					language="typescript"
...
// 2. Create PageSteerCore with controller
