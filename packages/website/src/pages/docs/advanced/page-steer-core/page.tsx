...
export default function PageSteerCoreDocs() {
	const { isZh } = useLanguage()

	return (
		<div>
			<h1 className="text-4xl font-bold mb-6">PageSteerCore</h1>

			<p className="text-xl text-gray-600 dark:text-gray-300 mb-8 leading-relaxed">
				{isZh
					? 'PageSteerCore 是不带 UI 的核心 Agent 类。用于需要自定义 UI 或无头运行的场景。'
					: 'PageSteerCore is the core Agent class without UI. Use it for custom UI or headless scenarios.'}
			</p>

			{/* When to use */}
			<section className="mb-10">
				<Heading id="when-to-use-pageagentcore">
					{isZh ? '何时使用 PageSteerCore' : 'When to Use PageSteerCore'}
				</Heading>
				<ul className="list-disc list-inside text-gray-600 dark:text-gray-400 space-y-2">
					...
					<li>
						{isZh
							? '将 PageSteer 嵌入其他 Agent 系统'
							: 'Embedding PageSteer in other agent systems'}
					</li>
				</ul>
			</section>
			...
			<section className="mb-10">
				...
				<CodeEditor
					language="typescript"
					code={`import { PageSteerCore } from '@page-steer/core'
import { PageController } from '@page-steer/page-controller'

const agent = new PageSteerCore({
...
const result = await agent.execute('Fill in the form with test data')`}
			...
			{/* Configuration */}
			<section className="mb-10">
				<Heading id="configuration">PageSteerCoreConfig</Heading>
				<p className="text-gray-600 dark:text-gray-400 mb-4">
					{isZh
						? 'PageSteerCoreConfig = AgentConfig & { pageController: PageController }。AgentConfig 包含以下配置项：'
						: 'PageSteerCoreConfig = AgentConfig & { pageController: PageController }. AgentConfig contains the following options:'}
				</p>
				...
				<APIReference
					properties={[
						...
						{
							name: 'customTools',
							type: 'Record<string, PageSteerTool | null>',
							status: 'experimental',
							description: isZh
			...
			<section className="mb-10">
				...
				<p className="text-gray-600 dark:text-gray-400 mb-4">
					{isZh ? (
						<>
							PageSteerCore 继承自 <TypeRef>EventTarget</TypeRef>，提供以下事件：
						</>
					) : (
						<>
							PageSteerCore extends <TypeRef>EventTarget</TypeRef> and provides the following
							events:
						</>
					)}
				</p>
