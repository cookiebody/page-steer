...
export default function PageSteerDocs() {
	const { isZh } = useLanguage()

	return (
		<div>
			<h1 className="text-4xl font-bold mb-6">PageSteer</h1>

			<p className="text-xl text-gray-600 dark:text-gray-300 mb-8 leading-relaxed">
				{isZh
					? 'PageSteer 是带有内置 UI 面板的完整 Agent 类。它继承自 PageSteerCore，并自动创建交互面板和 PageController。'
					: 'PageSteer is the complete Agent class with built-in UI panel. It extends PageSteerCore and automatically creates an interactive panel and PageController.'}
			</p>

			{/* When to use */}
			<section className="mb-10">
				<Heading id="when-to-use-pagesteer">
					{isZh ? '何时使用 PageSteer' : 'When to Use PageSteer'}
				</Heading>
				<p className="text-gray-600 dark:text-gray-400 mb-4">
					{isZh
						? '在大多数场景下，你应该使用 PageSteer。它提供了开箱即用的完整体验：'
						: 'In most cases, you should use PageSteer. It provides a complete out-of-the-box experience:'}
				</p>
			...
			<section className="mb-10">
				...
				<CodeEditor
					language="typescript"
					code={`import { PageSteer } from 'page-steer'
...
console.log(result.history) // Full execution history`}
			...
			<section className="mb-10">
				...
				<CodeEditor
					language="typescript"
					code={`class PageSteer extends PageSteerCore {
  panel: Panel
  ...
  constructor(config: PageSteerConfig)
				...
				<p className="text-gray-600 dark:text-gray-400 mt-4">
					{isZh ? (
						<>
							...
							，所有核心方法和事件都可用。配置项合并了{' '}
							<Link
								href="/advanced/page-steer-core#configuration"
								className="text-blue-600 dark:text-blue-400 hover:underline"
							>
								AgentConfig
							</Link>{' '}
			...
			<section className="mb-10">
				...
				<p className="text-gray-600 dark:text-gray-400 mb-4">
					{isZh
						? 'PageSteer 自动创建一个 Panel 实例。你可以通过 panel 属性控制 UI：'
						: 'PageSteer automatically creates a Panel instance. You can control the UI via the panel property:'}
				</p>
				<CodeEditor
					language="typescript"
...
agent.panel.dispose()`}
			...
			{/* Comparison with PageSteerCore */}
			<section className="mb-10">
				<Heading id="pageagent-vs-pageagentcore">
					{isZh ? 'PageSteer vs PageSteerCore' : 'PageSteer vs PageSteerCore'}
				</Heading>
