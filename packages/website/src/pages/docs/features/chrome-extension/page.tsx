...
export default function ChromeExtension() {
	...
	const chromeWebStoreUrl =
		'https://chromewebstore.google.com/detail/page-steer-ext/akldabonmimlicnjlflnapfeklbfemhj'
	const githubReleasesUrl = 'https://github.com/cookiebody/PageSteer/releases'
	...
	return (
		<div>
			...
			<p className="text-xl text-gray-600 dark:text-gray-300 mb-8 leading-relaxed">
				{isZh
					? '可选的 Chrome 扩展。PageSteer.js 继续负责页面内自动化；扩展 API 额外提供多页面任务、浏览器级控制，以及从浏览器外部发起任务的能力。'
					: 'An optional Chrome extension. PageSteer.js keeps handling in-page automation, while the extension API adds multi-page tasks, browser-level control, and tasks initiated from outside the browser.'}
			</p>
			...
			<div className="space-y-8 mt-8">
				...
				{/* Relationship with PageSteer.js */}
				<section>
					<Heading id="how-it-relates-to-page-steer-js" className="text-2xl font-bold mb-4">
						{isZh ? '与 PageSteer.js 的关系' : 'How It Relates to PageSteer.js'}
					</Heading>
					<div className="p-5 bg-gray-50 dark:bg-gray-800 rounded-lg space-y-3 text-gray-600 dark:text-gray-300">
						<p>
							{isZh
								? 'PageSteer.js 本身即可在页面内完成自动化。Chrome 扩展是可选的能力扩展。'
								: 'PageSteer.js already works for in-page automation. The Chrome extension is optional, not a dependency.'}
						</p>
						<p>
							{isZh
				...
				<section>
					...
					<p className="text-gray-600 dark:text-gray-300 mb-4">
						{isZh
							? '通过页面 JavaScript 调用 `window.PAGE_STEER_EXT`，你的应用可以发起跨页面任务并控制浏览器行为。'
							: 'By calling `window.PAGE_STEER_EXT` from page JavaScript, your app can trigger multi-page tasks and control browser behavior.'}
					</p>
...
// 2) 仅在可信应用中设置该 token
// 3) token 匹配后，扩展会暴露 window.PAGE_STEER_EXT

// ⚠️ 不要把 token 提供给不可信页面或脚本
localStorage.setItem('PageSteerExtUserAuthToken', '<从扩展中获取的-token>')`
								: `// 1) Get auth token from the extension side panel
// 2) Set it only in trusted applications
// 3) After token match, extension exposes window.PAGE_STEER_EXT

// ⚠️ Never provide the token to untrusted pages or scripts
localStorage.setItem('PageSteerExtUserAuthToken', '<your-token-from-extension>')`
						}
						language="javascript"
					/>
				</section>
				...
				<section>
					...
					<section className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
						...
						<p className="text-gray-600 dark:text-gray-300 mb-3 text-sm">
							{isZh
								? '如果你在使用 AI 编程助手（如 Cursor、GitHub Copilot），可以将以下文档链接提供给它，让它更好地理解和使用 Page Steer 扩展 API：'
								: 'If you are using an AI coding assistant (like Cursor, GitHub Copilot), share these documentation links with it for better understanding of Page Steer Extension API:'}
						</p>
						<div className="space-y-2">
							<a
								href="https://github.com/cookiebody/PageSteer/blob/main/packages/extension/docs/extension_api.md"
								target="_blank"
								rel="noopener noreferrer"
	...
	interface Window {
		...
		PAGE_STEER_EXT?: {
			...
			stop: () => void
					...
					<h3 className="text-xl font-semibold mt-6 mb-3">PAGE_STEER_EXT.execute(task, config)</h3>
...
const result = await window.PAGE_STEER_EXT.execute(
	'在 GitHub 上搜索 "page-steer" 并打开第一个结果',
	{
...
const result = await window.PAGE_STEER_EXT.execute(
	'Search for "page-steer" on GitHub and open the first result',
	{
		baseURL: 'https://api.openai.com/v1',
		apiKey: 'your-api-key',
		model: 'gpt-5.2',
...
console.log(result) // Task execution result`
					...
					<h3 className="text-xl font-semibold mt-6 mb-3">PAGE_STEER_EXT.stop()</h3>
					<p className="text-gray-600 dark:text-gray-300 mb-4">
...
window.PAGE_STEER_EXT.stop()`
				...
				<section>
					...
					>
						{isZh
							? '将 MultiPageSteer 集成你自己的插件'
							: 'Integrate MultiPageSteer into Your Extension'}
					</Heading>
					<p>@TODO</p>
					<p className="text-gray-600 dark:text-gray-300 mb-4">
						{isZh
						...
						<a
							href="https://github.com/cookiebody/PageSteer/blob/main/packages/extension/src/entrypoints/background.ts"
							target="_blank"
							rel="noopener noreferrer"
