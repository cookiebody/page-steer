import type { PageSteer as PageSteerType } from 'page-steer'
...
let pageSteerModule: Promise<typeof import('page-steer')> | null = null

function getInjection(useCN?: boolean) {
	const cdn = useCN ? CDN_DEMO_CN_URL : CDN_DEMO_URL

	const injection = encodeURI(
		`javascript:(function(){var s=document.createElement('script');s.src=\`${cdn}?t=\${Math.random()}\`;s.setAttribute('crossorigin', true);s.type="text/javascript";s.onload=()=>console.log('PageSteer script loaded!');document.body.appendChild(s);})();`
	)
...
export default function HeroSection() {
	...
	useEffect(() => {
		pageSteerModule ??= import('page-steer')
		pageSteerModule.then(() => setReady(true))
	}, [])

	const handleExecute = async () => {
		if (!task.trim() || !ready || !pageSteerModule) return

		const { PageSteer } = await pageSteerModule
		const win = window as any

		if (!win.pageSteer || win.pageSteer.disposed) {
			win.pageSteer = new (PageSteer as typeof PageSteerType)({
				interactiveBlacklist: [document.getElementById('root')!],
				language: language,

				instructions: {
					system: 'You are a helpful assistant on PageSteer website.',
					getPageInstructions: (url: string) => {
						return url.includes('page-steer') ? 'This is PageSteer demo page.' : undefined
					},
				},
		...
		await win.pageSteer.execute(task)
	}
										...
										<div className="space-y-4">
											<div className="relative">
												<input
													...
													data-page-steer-not-interactive
												/>
												<button
											...
											<p className="text-xs text-gray-500 dark:text-gray-400 text-left">
												{isZh ? (
													<>
														...
														<a
															href="https://github.com/cookiebody/PageSteer/blob/main/docs/terms-and-privacy.md#2-testing-api-and-demo-disclaimer--terms-of-use"
															target="_blank"
